...
import (
	...
	"github.com/cindysurjawann/gobookings/internal/forms"
...
// renders the make a reservation page and display form
...
// renders the room page and display form
...
// renders the search availability page and display form
...
// renders the contact page and display form
...
func (m *Repository) PostShowLogin(w http.ResponseWriter, r *http.Request) {
	//when login or logout, its good practice to call renew token for session
	...
	for _, x := range rooms {
		...
		for _, y := range restrictions {
			if y.ReservationID > 0 {
				//its a reservation (0 if not)
				for d := y.StartDate; d.After(y.EndDate) == false; d = d.AddDate(0, 0, 1) {
					reservationMap[d.Format("2006-01-2")] = y.ReservationID
				}
			} else {
				//its a block (0 if not)
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		...
		for name, value := range curMap {
			// ok will be false if the value isn't in the map
			if val, ok := curMap[name]; ok {
				//only pay attention to values > 0, and that aren't in the form post
